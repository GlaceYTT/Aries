...
client.on("messageCreate", async (message) => {
    ...
    if (command === "play") {
        ...
        try {
            await createPlaylist(message.author.id, playlistName);
            const embed = new EmbedBuilder()
                .setColor('#2b71ec')
                .setAuthor({ name: 'Playlist Created!', iconURL: 'https://cdn.discordapp.com/attachments/1278067218466340884/1278067312192393246/debts.gif?ex=66d411b0&is=66d2c030&hm=14f7b2f011869c90e86de8ad2c679bd6bd2edf0be77e4836edc2f9115c3e60f6&'})
                .setDescription(`**Playlist \`${playlistName}\` has been created!**`);
            message.reply({ embeds: [embed] });
        } catch (error) {
            message.reply("Error creating playlist: " + error.message);
        }
    }  else if (command === "add-song") {
        const [playlistName, ...songParts] = args;
        const song = songParts.join(" ");
        if (!playlistName || !song) return message.reply("Please provide both playlist name and song name.");
        
        try {
            await addSongToPlaylist(message.author.id, playlistName, song);
            const embed = new EmbedBuilder()
                .setColor('#2b71ec')
                .setAuthor({ name: 'Song Added!', iconURL: 'https://cdn.discordapp.com/attachments/1278067218466340884/1278067312192393246/debts.gif?ex=66d411b0&is=66d2c030&hm=14f7b2f011869c90e86de8ad2c679bd6bd2edf0be77e4836edc2f9115c3e60f6&' })
                .setDescription(`**Added \`${song}\` to playlist \`${playlistName}\`.**`);
            message.reply({ embeds: [embed] });
        } catch (error) {
            message.reply("Error adding song: " + error.message);
        }
        ...
        try {
            await removeSongFromPlaylist(message.author.id, playlistName, song);
            const embed = new EmbedBuilder()
                .setColor('#2b71ec')
                .setAuthor({ name: 'Song Removed!', iconURL: 'https://cdn.discordapp.com/attachments/1278067218466340884/1278067312192393246/debts.gif?ex=66d411b0&is=66d2c030&hm=14f7b2f011869c90e86de8ad2c679bd6bd2edf0be77e4836edc2f9115c3e60f6&'})
                .setDescription(`**Removed \`${song}\` from playlist \`${playlistName}\`.**`);
            message.reply({ embeds: [embed] });
        } catch (error) {
            message.reply("Error removing song: " + error.message);
        }
    }  else if (command === "delete-playlist") {
        const playlistName = args.join(" ");
        if (!playlistName) return message.reply("Please provide the name of the playlist to delete.");
        
        try {
            await deletePlaylist(message.author.id, playlistName);
            const embed = new EmbedBuilder()
                .setColor('#2b71ec')
                .setAuthor({ name: 'Playlist Deleted!', iconURL: 'https://cdn.discordapp.com/attachments/1278067218466340884/1278067312192393246/debts.gif?ex=66d411b0&is=66d2c030&hm=14f7b2f011869c90e86de8ad2c679bd6bd2edf0be77e4836edc2f9115c3e60f6&'})
                .setDescription(`**Deleted playlist \`${playlistName}\`.**`);
            message.reply({ embeds: [embed] });
        } catch (error) {
            message.reply("Error deleting playlist: " + error.message);
        }
    }  else if (command === "show=songs") {
        const playlistName = args.join(" ");
        if (!playlistName) return message.reply("Please provide the name of the playlist.");
        
        try {
            const songs = await getPlaylistSongs(message.author.id, playlistName);
            if (!songs || songs.length === 0) return message.reply("Playlist is empty or does not exist.");
            
            const embed = new EmbedBuilder()
                .setColor('#2b71ec')
                .setAuthor({ name: 'Playlist Songs', iconURL: 'https://cdn.discordapp.com/attachments/1278067218466340884/1278067312192393246/debts.gif?ex=66d411b0&is=66d2c030&hm=14f7b2f011869c90e86de8ad2c679bd6bd2edf0be77e4836edc2f9115c3e60f6&' })
                .setDescription(`**Songs in \`${playlistName}\`:**\n${songs.join("\n")}`);
            message.reply({ embeds: [embed] });
        } catch (error) {
            message.reply("Error fetching songs: " + error.message);
        }
    }  else if (command === "show-playlists") {
        try {
            const playlists = await getUserPlaylists(message.author.id);
            if (!playlists || playlists.length === 0) return message.reply("You have no playlists.");
            
            const embed = new EmbedBuilder()
                .setColor('#2b71ec')
                .setAuthor({ name: 'Your Playlists', iconURL: 'https://cdn.discordapp.com/attachments/1278067218466340884/1278067312192393246/debts.gif?ex=66d411b0&is=66d2c030&hm=14f7b2f011869c90e86de8ad2c679bd6bd2edf0be77e4836edc2f9115c3e60f6&' })
                .setDescription(`**Your Playlists:**\n${playlists.join("\n")}`);
            message.reply({ embeds: [embed] });
        } catch (error) {
            message.reply("Error fetching playlists: " + error.message);
        }
...
client.riffy.on("nodeConnect", node => {
    console.log('\x1b[35m[ MUSIC ]\x1b[0m', `\x1b[32mNode "${node.name}" connected.\x1b[0m`);
});
